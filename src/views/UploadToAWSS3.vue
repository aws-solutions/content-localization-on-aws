<template>
  <div>
      ...
      <b-container>
        <b-row v-if="hasAssetParam">
          <b-col>
            ...
            <b-button variant="primary" @click="runWorkflow"> 
              <!-- v-if="validForm"  -->
              Start Workflow
            </b-button>
              ...
              <b-form-group>
                ...
                <div v-if="enabledOperators.includes('Translate') && customTerminologyList.length > 0 && customTerminologyList.filter(x => x.SourceLanguageCode === sourceLanguageCode).length > 0">
                  <div v-if="customTerminology.length > 0">
                    ...
                    ({{ customTerminology.length }} selected)
                  </div>
                  <div v-else>
                    <b>Custom Terminologies:</b>
                  </div>
                  <b-form-select
                    v-model="customTerminology" :options="customTerminologyList.filter(x => x.SourceLanguageCode === sourceLanguageCode).map( x => { return {'text': x.Name + ' (' + x.TargetLanguageCodes + ')', 'value': {'Name': x.Name, 'TargetLanguageCodes': x.TargetLanguageCodes}}})"
                    multiple
                  >
                  </b-form-select>
    ...
    <b-container v-if="executed_assets.length > 0">
      ...
      >
        prevent="openWindow(data.item.state_machine_console_link)">{{ data.item.workflow_status }}
        <!-- </a> -->
...
export default {
    ...
    workflowConfig() {
      return {
        ...
        Configuration: {
          ...
          MediaconvertStage2: {
            Mediaconvert: { MediaType: "Video", Enabled: true },
          },
          ...
          defaultTextSynthesisStage2: {
            Polly: { MediaType: "Text", Enabled: false },
          },
          defaultVideoStage2: {
            faceDetection: {
              MediaType: "Video",
  ...
  methods: {
    listTerminologiesRequest: async function () {
      ...
      console.log(
        "curl -L -k -X GET -H 'Content-Type: application/json' -H 'Authorization: '" +
          token +
          " " +
          this.DATAPLANE_API_ENDPOINT +
      ...
      fetch(this.DATAPLANE_API_ENDPOINT + "translate/list_terminologies", {
        ...
        response
          ...
          .then((res) => {
            this.customTerminologyList = res.data[
    ...
    listVocabulariesRequest: async function () {
      ...
      fetch(this.DATAPLANE_API_ENDPOINT + "/transcribe/list_vocabularies", {
        ...
        response
          ...
          .then((res) => {
            this.customVocabularyList = res.data["Vocabularies"].map(
            ...
            if (
              this.customVocabularyList.filter(
    ...
    runWorkflow: async function (location) {
      ...
      if (this.hasAssetParam) {
        ...
        if (media_type.match(/image/g)) {
          ...
          if (this.customTerminology !== null) {
            data.Configuration.TranslateStage2.TranslateWebCaptions.TerminologyNames = JSON.stringify(
              { JsonList: this.customTerminology }
            );
          }
          ...
          if (this.existingSubtitlesFilename == "") {
            if (
              ...
              data.Configuration.WebCaptionsStage2.WebCaptions
            ) {
              delete data.Configuration.WebCaptionsStage2.WebCaptions
      ...
      //         console.log(
      //           "URL: " + this.WORKFLOW_API_ENDPOINT + "workflow/execution"
      //         );
      //         console.log("Data:");
      //         console.log(JSON.stringify(data));
      //         console.log(data);
    ...
    //     } else if (
    //       media_type.match(/video/g) ||
    //       this.valid_media_types.includes(s3key.split(".").pop().toLowerCase())
    //     ) {
    //       data = vm.workflowConfig;
    //       data["Input"] = {
    //         Media: {
